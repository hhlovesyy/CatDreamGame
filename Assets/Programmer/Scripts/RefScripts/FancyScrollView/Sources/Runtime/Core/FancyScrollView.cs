...
namespace FancyScrollView
{
    ...
    public abstract class FancyScrollView<TItemData, TContext> : MonoBehaviour where TContext : class, new()
    {
        ...
        protected int curFirstIndex = -1;
        //添加一个可绑定监听的事件
        public delegate void OnUpdatePosition(int firstIndex);
        public event OnUpdatePosition onPositionChange;
        void UpdatePosition(float position, bool forceRefresh)
        {
            ...
            curFirstIndex = firstIndex + 1;
            onPositionChange?.Invoke(curFirstIndex);
        ...
        {
            //Debug.LogError("firstIndex:" + firstIndex);
            //firstIndex + 1是第一个位置的关卡，只有这个位置的才会显示UI，其他的不会显示出来
            for (var i = 0; i < pool.Count; i++)
            {
                ...
                //my add :自己新加的，只有第一个位置的UI会被更新
                cell.UpdateFirstIndex(firstIndex + 1);
